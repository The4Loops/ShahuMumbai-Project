...
exports.createOrder = async (req, res) => {
  ...
  try {
    ...
    if (!dbReady(req) && devFakeAllowed()) {
      ...
      const total = Number((subtotal - Number(discount_total || 0) + Number(tax_total || 0) + Number(shipping_total || 0)).toFixed(2));
    ...
    // Load products (IDs must all be present)
    ...
    // Build items and totals (no stock checks here; we’ll enforce at payment time)
    let subtotal = 0;
    const normalizedItems = [];

    for (const it of items) {
      const p = byId.get(String(it.product_id));
      if (!p) return res.status(404).json({ ok: false, error: 'product_not_found', product_id: it.product_id });

      const inactive = (p.IsActive === 0 || p.IsActive === 'N' || p.IsActive === false);
      if (inactive) return res.status(400).json({ ok: false, error: 'product_inactive', product_id: p.Id });
    ...
    // Insert Order + OrderItems (no stock decrement)
    ...
    try {
      ...
      await tx.commit();
      return res.json({ ok: true, order_id: orderId, order_number: orderNumber, total, currency });
    } catch (inner) {
