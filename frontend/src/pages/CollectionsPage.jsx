...
export default function CollectionPage() {
  ...
  const categories = [
    { id: 1, name: "Shirts",   items: 45, color: "#E0E7FF", description: "Explore a wide range of stylish and comfortable shirts." },
    { id: 2, name: "Trousers", items: 30, color: "#FDE68A", description: "Premium trousers crafted for every occasion." },
    { id: 3, name: "Jackets",  items: 20, color: "#FECACA", description: "Stay warm and fashionable with our trendy jackets." },
    { id: 4, name: "Shoes",    items: 25, color: "#BBF7D0", description: "Step into comfort with our stylish footwear collection." },
  ];
  ...
  const pageUrl = `${baseUrl}/collections`; // this page is a generic category hub
  ...
  const collectionPageJsonLd = {
    ...
    url: pageUrl,
    description: "Browse Shahu Mumbai collections by category.",
  };

  const itemListJsonLd = {
    "@context": "https://schema.org",
    "@type": "ItemList",
    itemListElement: filteredCategories.map((c, i) => ({
      "@type": "ListItem",
      position: i + 1,
      name: c.name,
      url: `${pageUrl}#${encodeURIComponent(c.name)}`,
    })),
  };
  ...
  return (
    <Layout>
      <Helmet>
        <title>Shop by Category — Shahu Mumbai</title>
        <meta
          name="description"
          content="Find your favorite styles across shirts, trousers, jackets, and shoes at Shahu Mumbai."
        />
        <link rel="canonical" href={pageUrl} />
        <meta property="og:type" content="website" />
        <meta property="og:title" content="Shop by Category — Shahu Mumbai" />
        <meta
          property="og:description"
          content="Explore curated collections across categories."
        />
        <meta property="og:url" content={pageUrl} />
        <meta property="og:image" content={`${baseUrl}/og/collections.jpg`} />
